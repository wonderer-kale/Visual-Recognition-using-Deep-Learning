...
class LinearLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        weight = (1 - process)
        return [weight * group[
            'initial_lr'] for group in self.optimizer.param_groups]
...
class CosineAnnealingRestartCyclicLR(_LRScheduler):
    ...
    def __init__(self,
        ...
        super(CosineAnnealingRestartCyclicLR, self).__init__(
...
class LinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    and base_lr followed by a cosine annealing schedule
    between base_lr and eta_min.
    .. warning::
        It is recommended to call :func:`.step()` for
        :class:`LinearWarmupCosineAnnealingLR`
        after each iteration as calling it after each epoch
        will keep the starting lr at warmup_start_lr
        for the first epoch which is 0 in most cases.
    .. warning::
        ...
        this scheduler instead of :func:`get_lr()`.
        Though this does not change the behavior of the scheduler, when passing
        epoch param to :func:`.step()`, the user should call the
    ...
    def _get_closed_form_lr(self) -> List[float]:
        """Called when epoch is passed as a param to
        the `step` function of the scheduler."""
        if self.last_epoch < self.warmup_epochs:
            return [
...
def linear_warmup_decay(warmup_steps, total_steps, cosine=True, linear=False):
    """Linear warmup for warmup_steps, optionally with
    cosine annealing or linear decay to 0 at total_steps."""
